...
import { postToPlatform, PlatformPost } from './utils/publishing';
    ...
    try {
      ...
      const results = await Promise.all(
        platforms.map(async (platform: string) => {
          try {
            const postData: PlatformPost = {
              content,
              media,
            };
            
            // Fetch connection settings to get subreddit/pageId/etc.
            const connections = (await kv.get(`project:${projectId}:connections`)) || [];
            const connection = connections.find((c: any) => c.platform === platform);
            
            const options: Record<string, any> = {};
            if (connection) {
                if (platform === 'reddit' && connection.subreddit) options.subreddit = connection.subreddit;
                if (platform === 'facebook' && connection.pageId) options.pageId = connection.pageId;
                if (platform === 'pinterest' && connection.boardId) options.boardId = connection.boardId;
                // For pinterest/instagram we also need imageUrl from media
                if ((platform === 'pinterest' || platform === 'instagram') && media && media.length > 0) {
                    options.imageUrl = media[0];
                }
            }

            return await postToPlatform(platform, projectId, postData, options);
          } catch (error: any) {
            return {
              platform,
              success: false,
